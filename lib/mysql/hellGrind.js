...
function makeHellgrindFile(rlBefore) {
  rlBefore.close();
...
hellGrind.main = function main(isAwal) {
  mysql.connection()
    .then(() => {
      const rl = readline.createInterface({
        input: process.stdin,
      ...
      // rl.prompt();
      rl.on('line', (input) => {
        console.log('ini prefix', input);
        rl.close();
        // process.exit();
        // if (input.toLowerCase() === 'exit') process.exit();
        // if (input.toLowerCase() === 'make') makeHellgrindFile(rl);
        // if (input.toLowerCase() === 'model')rl.close(); runModel();
      });
    })
    .catch(() => {
      process.stdout.write('Sorry, database cannot connected.\n');
