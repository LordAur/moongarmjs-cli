...
Query.buildSeeder = function buildSeeder(json, data, callback) {
  ...
  Object.keys(data).forEach((field, fieldIndex) => {
    ...
    if ((fieldIndex + 1) === Object.keys(data).length) {
      for (let i = 0; i < 1; i += 1) {
        const valueQuery = [];
        if (typeof data[field] === 'object') {
          Object.keys(data[field]).forEach((batchKey) => {
            if (batchKey === 'randomStringValue' && data[field][batchKey] === true && data[field].randomLength === undefined) {
              const stringRandom = randomstring.generate(7);
              valueQuery.push(`'${stringRandom}'`);
            }

            if (batchKey === 'randomStringValue' && data[field][batchKey] === true && data[field].randomLength !== undefined) {
              const stringRandom = randomstring.generate(data[field].randomLength);
              valueQuery.push(`'${stringRandom}'`);
            }

            if (batchKey === 'randomIntegerValue' && data[field][batchKey] === true) {
              const integerRandom = Math.random();
              valueQuery.push(`${integerRandom}`);
            }
          });
        } else if (typeof data[field] !== 'object' && Number.isInteger(data[field])) {
          Object.keys(data).forEach((key) => {
            valueQuery.push(`'${data[key]}'`);
          });
        } else if (typeof data[field] !== 'object' && !Number.isInteger(data[field])) {
          Object.keys(data).forEach((key) => {
            valueQuery.push(`'${data[key]}'`);
