...
Query.buildSeeder = function buildSeeder(tableName, data, callback) {
  const insertQuery = [];
  const valueQuery = [];

  insertQuery.push(`\`${Object.keys(data)}\``);
  if (Number.isInteger(data[Object.keys(data)])) {
    valueQuery.push(`${data[Object.keys(data)]}`);
  } else {
    valueQuery.push(`'${data[Object.keys(data)]}'`);
  }

  return callback(`INSERT INTO \`${tableName}\`(${insertQuery.join(', ')}) VALUES (${valueQuery.join(', ')})`);
};
